...
faq:
  ...
  - q: "Claude Code를 실무에서 쓸 때 가장 큰 한계는 무엇인가요?"
    a: "정확한 수치나 조건을 대화로만 전달하면 임의로 수정하는 경우가 있습니다. 또 컨텍스트가 길어지면 테스트와 프로덕션 환경을 혼동하며, 첫 초안의 30-40%에 사소한 버그가 있습니다."
  - q: "Claude가 만든 코드를 그대로 배포해도 되나요?"
    a: "안 됩니다. 첫 초안은 엣지 케이스 누락, API 형식 변경, 조건 반전 같은 버그가 자주 발생합니다. 반드시 테스트를 돌리고 검증한 뒤 배포해야 합니다."
  - q: "Claude가 요구사항을 잘못 해석하지 않게 하려면 어떻게 해야 하나요?"
    a: "정확한 숫자, 파라미터, 조건은 대화가 아닌 참조 문서나 설정 파일에 고정시켜야 합니다. 특히 트레이딩처럼 작은 수치 차이가 결과를 뒤집는 영역은 필수입니다."
  - q: "Claude가 틀렸을 때 어떻게 피드백해야 효율적인가요?"
    a: "장황한 설명이나 감정 표현 없이 'Y가 아니라 X여야 한다'고 즉시 방향만 수정하세요. 토큰 비용을 아끼고 대화 길이로 인한 컨텍스트 혼동도 줄일 수 있습니다."
  - q: "Claude Code는 어떤 사용자에게 가장 적합한가요?"
    a: "여러 파일을 동시에 수정하는 중대형 프로젝트, 멀티 에이전트 협업, 야간 자율 실행 같은 DevOps 자동화를 다루는 팀에 적합합니다. 단발성 스크립트 작성에는 과합니다."
  - q: "Claude의 다중 파일 편집은 다른 AI 도구와 어떻게 다른가요?"
    a: "여러 파일과 모듈, 설정을 한 번에 수정하면서 파일 간 의존 관계를 추적합니다. 'A는 바꿨는데 B를 빼먹는' 초보적 실수가 거의 없어 리팩토링 작업에서 차별화됩니다."
...
- [회사에서 코드 에이전트로 Claude Code를 평가 중입니다. 경험과 교훈?](https://www.reddit.com/r/ClaudeAI/comments/1qdh2rc/evaluating_claude_code_as_a_code_agent_in_a/?tl=ko)
- [Maximize Claude Code as Your Personal Coach (It also ... - YouTube](https://www.youtube.com/watch?v=YIDeIjqb9A0)
- [(4편) Claude Code를 쓰면서 느낀 점 - 브런치](https://brunch.co.kr/@insaboy/326)
