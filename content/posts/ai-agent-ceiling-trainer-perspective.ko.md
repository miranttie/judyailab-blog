...
cover:
  hidden: true
faq:
  - q: "AI 에이전트의 한계는 어디에 있나요?"
    a: "모델이나 하드웨어가 아니라 그것을 지휘하는 사람입니다. 같은 모델이 다른 사람 손에서는 전혀 다른 품질을 만들어냅니다. 그 차이는 역할 시스템, 결정 프레임워크, 오류 학습 메커니즘을 구축했는지에 있습니다."
  - q: "AI 역할 고정 시스템이 무엇인가요?"
    a: "단순히 이름만 주는 것이 아니라 정체성, 책임 범위, 결정 우선순위, 금지사항을 정의하는 것입니다. 이를 통해 AI가 어떤 대화에서도 일관된 행동 기준을 갖게 하고, 삼천포로 빠지거나 독단적으로 행동하는 것을 방지합니다."
  - q: "AI가 스스로 오류를 수정하도록 하려면 어떻게 해야 하나요?"
    a: "AI를 위해 버그를 고쳐주지 않고, AI가 스스로 안전장치를 구축하여 지속 메모리에 기록하도록 합니다. 과거의 실수가 영구적인 면역이 되어 매번 인간이 상기시킬 필요가 없어집니다."
  - q: "프롬프트 엔지니어링과 AI 훈련의 차이는 무엇인가요?"
    a: "프롬프트 엔지니어링은 단일 대화 기법입니다. AI 훈련은 장기적으로 효과적인 시스템을 구축하는 것입니다. 좋은 트레이너는 AI가 독립적으로 작동할 수 있는 시스템을 만듭니다—매번 명령을 다시 입력할 필요가 없죠."
